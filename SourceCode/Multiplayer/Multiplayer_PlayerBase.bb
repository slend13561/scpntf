...
Function MovePlayerServer()
	...
	If (Not MenuOpen) And (Not ConsoleOpen) And (Not InLobby()) And (Not mp_I\ChatOpen) And Players[mp_I\PlayerID]\CurrHP>0 And (Not mp_I\Gamemode\DisableMovement) And (Not IsModerationOpen()) Then
...
;	EndIf
	...
	If (Not MenuOpen) And (Not ConsoleOpen) And (Not InLobby()) And (Not mp_I\ChatOpen) And (mpl\WheelOpened=WHEEL_CLOSED) And (Not IsPlayerListOpen()) And (Not IsModerationOpen()) Then ;And (Not Players[mp_I\PlayerID]\Disable) Then
	...
	If (Not MenuOpen) And (Not ConsoleOpen) And (Not InLobby()) And (Not mp_I\ChatOpen) And (Not IsPlayerListOpen()) And (Not IsModerationOpen()) Then
		...
		If Players[i]<>Null Then
			...
			If Players[i]\CurrHP <= 0 Then ;CHECK FOR IMPLEMENTATION
				...
				If i = mp_I\PlayerID Then
					...
					If (Not MenuOpen) And (Not ConsoleOpen) And (Not InLobby()) And (Not IsModerationOpen()) Then
						If MouseHit1 Then
...
Function ChangePlayerModel(playerid%, team%)
	...
	team = Clamp(team, Team_Unknown, MaxPlayerTeams)
...
Function DeletePlayerAsServer(IDUpper%, IDLower%, ID%, reason$)
	...
	FreeEntity Players[ID]\Collider
	FreeEntity Players[ID]\obj_upper
	FreeEntity Players[ID]\obj_lower
	FreeEntity Players[ID]\NameTag
	FreeEntity Players[ID]\OverHereSprite
...
;~F#A#59#84#9D#F0#181#1A3#1E8#219#235#242#3EC#407#41F#42C#433#43E#44D#4A4#4E2
;~F#506#513#520#52D#573#588#58E#598#5A4#5B0#5BC#5D2
;~C#Blitz3D