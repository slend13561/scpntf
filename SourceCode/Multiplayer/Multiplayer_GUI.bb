...
Function DrawGUIMP()
	...
	If MenuOpen Lor ConsoleOpen Lor InLobby() Lor mp_I\ChatOpen Lor IsPlayerListOpen() Lor IsModerationOpen() Then
		ShowPointer()
	Else
		HidePointer()
	EndIf
	...
	If HUDenabled And (Not InLobby()) And Players[playerid]\Team > Team_Unknown Then ;CHECK FOR IMPLEMENTATION
		...
		If Players[playerid]\CurrStamina < 100.0 Then
			...
			If Players[playerid]\CurrStamina <= 20.0 Then
				Color 255, 0, 0
			Else
				Color 255, 255, 255
			EndIf
			...
			If Players[playerid]\CurrStamina <= 0.0 Then
				Color 255, 0, 0
			Else
				Color 255, 255, 255
			EndIf
		...
		Select mp_I\Gamemode\ID
			Case Gamemode_Waves
				...
				If (mp_I\Gamemode\Phase Mod 2) = 1 Then
					Color 0,255,0
					SetFont fo\Font[Font_Digital_Medium]
					If TurnIntoSeconds(mp_I\Gamemode\PhaseTimer) < 10 Then
						TextWithAlign x,y,"0:0"+TurnIntoSeconds(mp_I\Gamemode\PhaseTimer),2,0
					Else
						TextWithAlign x,y,"0:"+TurnIntoSeconds(mp_I\Gamemode\PhaseTimer),2,0
					EndIf	
				...
				Else
					...
					Next
				EndIf
				If mp_I\Gamemode\Phase = Waves_End Then
			...
			Case Gamemode_Deathmatch
				...
				If mp_I\Gamemode\PhaseTimer > 0 Then
					...
					SetFont fo\Font[Font_Digital_Medium]
					If TurnIntoSeconds(mp_I\Gamemode\PhaseTimer) < 10 Then
						TextWithAlign x,y+40,"0:0"+TurnIntoSeconds(mp_I\Gamemode\PhaseTimer),2,0
					Else
						TextWithAlign x,y+40,"0:"+TurnIntoSeconds(mp_I\Gamemode\PhaseTimer),2,0
					EndIf
					If mp_I\Gamemode\Phase = Deathmatch_Game Then
						Color 0,Min(mp_I\Gamemode\PhaseTimer,127.5)*2,0
						SetFont fo\Font[Font_Digital_Large]
...
;~F#C4#DD#116#11D#17B#1AD#35C#364
;~C#Blitz3D