...
Function ListServers()
	...
	Local tcp = OpenTCPStream(ServerAddress, GetPortForProtocol(Protocol))
	If tcp<>0 Then
		WriteLine tcp, "GET "+Protocol+"://"+ServerAddress$+"/"+PhpAddress$+"/?action=listservers HTTP/1.1"
		WriteLine tcp, "Host: "+ServerAddress
		WriteLine tcp, "User-Agent: SCPNTF"
		WriteLine tcp, "Connection: Close"
		WriteLine tcp, ""
...
;	
;	mp_I\Offline = False
;	Local tcp = OpenTCPStream(ServerAddress, 80)
;	If tcp<>0 Then
;		WriteLine tcp, "GET http://"+ServerAddress$+"/"+PhpAddress$+"/?action=getip HTTP/1.1"
;		WriteLine tcp, "Host: "+ServerAddress
;		WriteLine tcp, "User-Agent: SCPNTF"
;		WriteLine tcp, "Connection: Close"
;		WriteLine tcp, ""
