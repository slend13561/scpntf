...
Function MPMainLoop()
	...
	While (ft\accumulator>0.0)
		...
		PlayGunSoundsMP()
		AnimateGunsServer()
		If Players[mp_I\PlayerID]\CurrHP > 0 And (Not ConsoleOpen) And (Not mp_I\ChatOpen) And (Not MenuOpen) And (Not InLobby()) And (Not IsModerationOpen()) Then
...
Function RecvDataServer()
	...
	Local hasClosestItem%,ItemInSpecialSlot%,ItemDeleted%,ItemID%,IsItemPicked%,it.Items,p.Particles,fb.FuseBox,lg.LeverGen,bg.ButtonGen
					...
					Select currMSGSync
						...
						Case PACKET_PLAYER
							...
							If Players[TempID]<>Null Then
								...
								If prevusername <> Players[TempID]\Name Then
									FreeEntity Players[TempID]\NameTag
...
Function RecvDataClient()
	...
	Local temp%
	Local it.Items,hasItem%,deleteitem%,itID%,itX#,itY#,itZ#,itName$,itTempName$,itYaw#,itFound.Items,fb.FuseBox,ge.Generator,dbr.DamageBossRadius,pg.ParticleGen,lg.LeverGen,bg.ButtonGen
...
;					EndIf
;				Next
;				;May need to be moved to another package?
...
;				For i=0 To (mp_I\MaxPlayers-1)
;					ttmp% = ReadByte(mp_I\Server)
;					If ttmp% And i<>mp_I\PlayerID Then
;						NewMSG = ReadLine(mp_I\Server)
;						If NewMSG <> "" Then
;							AddChatMSG(NewMSG,i)
;						EndIf
...
;							EndIf
			...
			Case PACKET_GAMEMODEUTIL
				...
				If mp_I\Gamemode\ID = Gamemode_Waves Then
					If (mp_I\Gamemode\Phase = Waves_StartGame Or (mp_I\Gamemode\Phase Mod 2) <> 0) And mp_I\Gamemode\PhaseTimer > 0 Then
...
;					;[Block]
;					WriteByte mp_I\Server,PACKET_CHATMSG
;					
;					For j=0 To (mp_I\MaxPlayers-1)
;						If Players[j]<>Null Then
...
;									WriteLine mp_I\Server,Players[j]\SendChatMSG ;Think about a sort of compression algorithm if needed, but currently it is not I think - ENDSHN
;								Else
;									WriteLine mp_I\Server,"" ;Seems redundant at first, but that will just send 1 single Byte (a termination symbol), which is equivilant to WriteByte(mp_I\Server,0)
;								EndIf
;							EndIf
;						Else
...
Function SyncClient()
	...
	Local getconn,TempRead$,i%,k%,ttmp%,yaw#,fb.FuseBox,lg.LeverGen,bg.ButtonGen
...
;~F#2B#63#6F#7B#81#8F#9C#A7#AD#C5#DC#F2#FE#115#126#13E#152#15B#164#174
;~F#1CA#22D#25A#265#2AE#2F3#318#326#32F#42B#507#518#5A3#5B5#5B9#66C#681#69C#840#845
;~F#947#972#9B8#9D9#9ED#A36#A45#A53#A63#B34#B3D#BA9#BBD#BDA#BF6#C19#C3D#C84#C8C#D30
;~F#D3B#D46
;~C#Blitz3D