...
Function UpdateVolumeFog()
	...
	For vf.VolumeFog = Each VolumeFog
		...
		ElseIf vf\deadTime# < -99.9 And vf\deadTime# > -300.0
			...
			vf\deadTime# = vf\deadTime# - FPSfactor
		Else
			If vf\obj% <> 0 Then FreeEntity vf\obj% : vf\obj% = 0
			Delete vf
		EndIf
	Next
...
Function SZL_LoadEntitiesForZone()
	...
	If ApacheObj<>0
		FreeEntity ApacheObj : ApacheObj=0
		FreeEntity ApacheRotorObj : ApacheRotorObj=0
	EndIf
	If Room2slCam<>0
		FreeEntity Room2slCam : Room2slCam=0
	EndIf
	If N966Obj<>0
		FreeEntity N966Obj : N966Obj=0
	EndIf
