...
;					EndIf
			...
			Case MenuTab_Singleplayer
				...
				If m_I\CurrentSave <> "" And DrawButton(x+20*MenuScale,y+20*MenuScale,width-40*MenuScale,70*MenuScale, Upper(GetLocalString("Menu", "continue")), 2) Then
					For sa = Each Save
					...
					Next
				EndIf
				If DrawButton(x+20*MenuScale,y+(20 + (80 * (m_I\CurrentSave <> "")))*MenuScale,width-40*MenuScale,70*MenuScale, Upper(GetLocalString("Menu", "newgame")), 2) Then
					RandomSeed = ""
					If Rand(15)=1 Then 
						Select Rand(15)
							Case 1 
					...
					MainMenuTab = MenuTab_NewGame
				EndIf
				If DrawButton(x+20*MenuScale,y+(100 + (80 * (m_I\CurrentSave <> "")))*MenuScale,width-40*MenuScale,70*MenuScale, Upper(GetLocalString("Menu", "loadgame")),2) Then
					LoadSaveGames()
...
;				EndIf
			...
			Case MenuTab_HostServer
				...
				If MouseHit1 Then
					If MouseOn(x + 400 * MenuScale, y + 60 * MenuScale, 30 * MenuScale, 30 * MenuScale) Then
						PlaySound_Strict ButtonSFX
						mp_I\PasswordVisible = Not mp_I\PasswordVisible
					EndIf
				EndIf
				
				If DrawButton(x+180*MenuScale, y+95*MenuScale, 170*MenuScale, 30*MenuScale, mp_O\MapInList\Name, False) Then
					MainMenuTab = MenuTab_SelectMPMap
				EndIf
				
				If DrawButton(x+180*MenuScale, y+135*MenuScale, 170*MenuScale, 30*MenuScale, mp_O\Gamemode\name, False) Then
					MainMenuTab = MenuTab_SelectMPGamemode
				EndIf
				
				If mp_O\Gamemode\ID = Gamemode_Waves Then
				If DrawButton(x + 400 * MenuScale, y + 135 * MenuScale, 170*MenuScale, 30*MenuScale, GetLocalString("Serverlist", "gamemode_options"),False,False,True,3,MainMenuTab,1) Then
					MainMenuTab = MenuTab_MPGamemodeSettings
				EndIf
				EndIf	
				
				mp_O\MaxPlayers = Max(InputBox(x+250*MenuScale, y+175*MenuScale, 100*MenuScale, 30*MenuScale, mp_O\MaxPlayers, 4, 4, MainMenuTab),0)
				If SelectedInputBox<>4 Then
					If mp_O\MaxPlayers < 2 Then mp_O\MaxPlayers = 2
					If mp_O\MaxPlayers > MaxPlayers Then mp_O\MaxPlayers = MaxPlayers ;can be changed in the future or such, but higher values could maybe cause issues with syncing as well while updating the server - ENDSHN
				EndIf
				
				mp_O\TimeOut = Max(InputBox(x+250*MenuScale, y+215*MenuScale, 100*MenuScale, 30*MenuScale, mp_O\TimeOut, 5, 5, MainMenuTab),0)
				If SelectedInputBox<>5 Then
					If mp_O\TimeOut < 1 Then mp_O\TimeOut = 1
					If mp_O\TimeOut > 30 Then mp_O\TimeOut = 30
				EndIf
				
				mp_O\LocalServer =  DrawTick(x + 330 * MenuScale, y + 260 * MenuScale, mp_O\LocalServer, False, 2, MainMenuTab, 1)
				...
				If DrawButton(x + 420 * MenuScale, y + height + 20 * MenuScale, 160 * MenuScale, 70 * MenuScale, Upper(GetLocalString("Serverlist","create_server")),False,False,True,6,MainMenuTab,1) Then
					If mp_O\MaxPlayers < 2 Then mp_O\MaxPlayers = 2
					If mp_O\MaxPlayers > MaxPlayers Then mp_O\MaxPlayers = MaxPlayers
					If mp_O\TimeOut < 1 Then mp_O\TimeOut = 1
					If mp_O\TimeOut > 30 Then mp_O\TimeOut = 30
					CreateServer()
					NTF_GameModeFlag = 2
			...
			Case MenuTab_SelectMPMap
				...
				For mfl = Each MapForList
					If DrawButton(x + 20 * MenuScale, y + 20 * MenuScale, 170*MenuScale, 25*MenuScale, mfl\Name, False, False, True, i+1, MainMenuTab) Then
						mp_O\MapInList = mfl
						For mgm = Each MultiplayerGameMode
							If mgm\name = Piece(mp_O\MapInList\Gamemodes,1,"|") Then
								mp_O\Gamemode = mgm
								Exit
							EndIf
						Next
			...
			Case MenuTab_SelectMPGamemode
				...
				For mgm = Each MultiplayerGameMode
					...
					Next
					If mapHasGM Then
						If DrawButton(x + 20 * MenuScale, y + 20 * MenuScale, 170*MenuScale, 25*MenuScale, mgm\name, False, False, True, i+1, MainMenuTab) Then
							mp_O\Gamemode = mgm
							MainMenuTab = MenuTab_HostServer
						EndIf
						y=y+30*MenuScale
...
;				Next
			...
			Case MenuTab_Singleplayer
				...
				y = y + height + 20 * MenuScale
				width = 580 * MenuScale
				height = 200 + (80 * (m_I\CurrentSave <> "")) * MenuScale
...
;				EndIf
			...
			Case MenuTab_Serverlist
				...
				width = 500*MenuScale
				DrawFrame(x, y, width, height)
				Text(x + width / 2, y + height / 2, GetLocalString("Serverlist", "servers"), True, True)
				;Server gamemode
				x = x + width
				width = 200 * MenuScale
				DrawFrame(x, y, width, height)
				Text(x + width / 2, y + height / 2, GetLocalString("Serverlist", "gamemode"), True, True)
				;Server map
				x = x + width
				width = 200 * MenuScale
				DrawFrame(x, y, width, height)
				Text(x + width / 2, y + height / 2, GetLocalString("Serverlist", "map"), True, True)
				;Player amount
				x = x + width
				width = 150 * MenuScale
				DrawFrame(x, y, width, height)
				Text(x + width / 2, y + height / 2, GetLocalString("Serverlist", "players"), True, True)
				;Server ping
				x = x + width
				width = 40 * MenuScale
				DrawFrame(x, y, width, height)
					...
					If i <= mp_I\ServerListAmount
						For se = Each Server
							If se\ID = i Then
								...
								Color 255,255,255
								For mgm = Each MultiplayerGameMode
									If mgm\ID = se\gamemode Then
										Text(x + width / 2, y + height / 2, mgm\name, True, True)
										Exit
									EndIf
								Next
								x = x + width
			...
			Case MenuTab_HostServer
				...
				Text (x + 20 * MenuScale, y + 20 * MenuScale, "Server name:")
				
				Text (x + 20 * MenuScale, y + 60 * MenuScale, "Password:")
				
				DrawImage mp_I\PasswordIcon, x + 400 * MenuScale, y + 60 * MenuScale, (mp_I\PasswordVisible)
				If MouseOn(x + 400 * MenuScale, y + 60 * MenuScale, 30 * MenuScale, 30 * MenuScale) Then
					Color 150,150,150
				Else
					Color 255,255,255
				EndIf
				Rect x + 400 * MenuScale, y + 60 * MenuScale, 30 * MenuScale, 30 * MenuScale, False
				
				Color 255,255,255
				
				;Text (x + 20 * MenuScale, y + 100 * MenuScale, "Port (UDP):")
				
				Text (x + 20 * MenuScale, y + 100 * MenuScale, "Selected map:")
				
				Text (x + 20 * MenuScale, y + 140 * MenuScale, "Selected gamemode:")
				
				Text (x + 20 * MenuScale, y + 180 * MenuScale, "Maximum players:")
				
				Text (x + 20 * MenuScale, y + 220 * MenuScale, "Server timeout (secs):")
				
				Text (x + 20 * MenuScale, y + 260 * MenuScale, "Private server:")
				If MouseOn(x+20*MenuScale,y+260*MenuScale,400*MenuScale,20*MenuScale)
					DrawOptionsTooltip("private")
				EndIf
				;[End Block]
			Case MenuTab_SelectMPMap,MenuTab_SelectMPGamemode
				;[Block]
				x = 59 * MenuScale
				y = 286 * MenuScale
				...
				Color(255, 255, 255)
				If MainMenuTab=MenuTab_SelectMPMap Then
					SetFont fo\Font[Font_Menu]
			...
			Case MenuTab_MPGamemodeSettings
				...
				Select mp_O\Gamemode\ID
					Case Gamemode_Waves
						...
						Text (x + 20 * MenuScale, y + 20 * MenuScale, "Difficulty:")
						
						y=y+50*MenuScale
						
						Text (x + 20 * MenuScale, y + 20 * MenuScale, "Amount of waves:")
						;[End Block]
				End Select
				;[End Block]
		End Select
